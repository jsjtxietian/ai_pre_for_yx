...
"知识权重"提前在海量数据上训练好，日常使用时，只是在调用已有的权重
...
Transformer 内部最重要的机制：**Self-Attention** —— 全局视野，复杂度 **O(N²)**
...
<div v-click class="p-3 bg-gray-50 rounded-lg border border-gray-200 text-center">

$$\text{Attention}(Q, K, V) = \text{softmax}\!\left(\frac{QK^\top}{\sqrt{d_k}}\right) V$$

<div class="text-xs opacity-60 mt-2">小明的例子里，"他" 该看向谁？—— 由 <strong>Q</strong> 和 <strong>K</strong> 的匹配度决定</div>
</div>

<div v-click class="text-sm mt-1">
<span v-mark="{ at: 4, type: 'highlight', color: '#f59e0b' }">任意两位置都可能相关</span> —— 不预设"只有附近重要"
</div>
...
<div class="p-3 bg-gray-50 rounded-lg border border-gray-200 text-sm font-serif italic leading-relaxed mt-2">
许多年之后，面对行刑队，奥雷良诺·布恩地亚上校将会回想起，他父亲带他去见识 <span class="not-italic font-mono text-sm px-2 py-0.5 bg-teal-100 border border-teal-300 rounded ml-1">___</span>
</div>

<div class="grid grid-cols-5 gap-4 mt-3 items-start">

<div class="col-span-3">
<div class="text-[11px] opacity-50 mb-1">"见识" 之后，模型对每个候选 Token 的预测概率</div>
<div class="flex items-end gap-1.5" style="height:80px">
  <div class="flex-1 bg-teal-400 rounded-t" style="height:76px"></div>
  <div class="flex-1 bg-sky-300 rounded-t" style="height:8px"></div>
  <div class="flex-1 bg-violet-300 rounded-t" style="height:6px"></div>
  <div class="flex-1 bg-rose-300 rounded-t" style="height:5px"></div>
  <div class="flex-1 bg-amber-300 rounded-t" style="height:4px"></div>
</div>
<div class="flex gap-1.5 mt-1 text-[10px] text-center">
  <div class="flex-1"><strong class="text-teal-700">冰块</strong><br/><span class="opacity-50">94.2%</span></div>
  <div class="flex-1 text-gray-400">大海<br/><span class="opacity-50">2.8%</span></div>
  <div class="flex-1 text-gray-400">奇迹<br/><span class="opacity-50">1.1%</span></div>
  <div class="flex-1 text-gray-400">死亡<br/><span class="opacity-50">0.5%</span></div>
  <div class="flex-1 text-gray-400">黄金<br/><span class="opacity-50">0.2%</span></div>
</div>
<div v-click class="text-[10px] opacity-60 mt-2 italic">《百年孤独》原文是"去见识冰" —— 冰块高概率，来自训练语料</div>
</div>

<div class="col-span-2 space-y-2 text-xs">

<div v-click class="p-2 bg-gray-50 rounded border border-gray-200 text-[11px] leading-snug">
<strong>Temperature（温度）</strong>：采样前调整概率分布的"尖锐度"的参数
</div>

<div v-click class="p-2 bg-teal-50 rounded border border-teal-200">
<strong>T=0（贪心）</strong> —— 每次选最高概率，永远是"冰块"
</div>

<div v-click class="p-2 bg-amber-50 rounded border border-amber-200">
<strong>T=0.7</strong> —— 冰块大概率选中；大海、奇迹也有机会
</div>
...
<div v-click class="mt-3 flex gap-4 text-sm">
<div class="flex-1 p-2 bg-sky-50 rounded border border-sky-200 text-center"><strong>T 接近 0</strong> —— 冷峻、精确、复读机</div>
<div class="flex-1 p-2 bg-amber-50 rounded border border-amber-200 text-center"><strong>T 接近 1</strong> —— 热情、发散、艺术家</div>
</div>

<div v-click class="mt-2 text-[11px] opacity-60 text-center">其他采样策略：Top-K · Top-P (Nucleus) · Min-P · Beam Search ……</div>
...
多请求并行时计算顺序不固定，微小差异同样层层放大（动态 Batching、量化、专家丢弃等）。

</div>

</div>

<div v-click class="mt-4 p-2 bg-gray-50 rounded-lg border border-gray-200 text-sm">
<strong>LLM 是一个自回归的概率预测引擎</strong> —— 学的是词语共现的统计规律，不是显式的知识检索。
</div>

<SlideRefs :refs="[
  { label: 'Defeating Nondeterminism in LLM Inference', url: 'https://thinkingmachines.ai/blog/defeating-nondeterminism-in-llm-inference/' }
]" />
...
问："UE 里有没有能实现某某功能的函数？"<br/>
→ 它不查文档，直接给你写一段"看起来完全正确"的示例代码 —— 但那个函数根本不存在。
...
预训练出来的 LLM 只是个"文字续写机" —— <strong>SFT + RL</strong> 等技巧才把它调教成 ChatGPT

<div class="mt-3 text-xs opacity-60">示例：同一个问题 <span class="font-mono bg-gray-100 px-1.5 py-0.5 rounded">"1+1 等于几？"</span> 在三个阶段的模型眼里：</div>
...
<div v-click class="p-3 bg-gray-50 rounded-lg border border-gray-300">
  ...
  <div class="text-[11px] mt-2 opacity-60"> 海量互联网文本 </div>
</div>

<div v-click class="p-3 bg-teal-50 rounded-lg border border-teal-300">
  <div class="text-xs font-mono opacity-60 mb-1"> SFT </div>
  ...
  <div class="text-[11px] mt-2 opacity-60"> 标注员写的高质量 Q&A，学指令格式、学口吻等 </div>
</div>

<div v-click class="p-3 bg-amber-50 rounded-lg border border-amber-300">
  <div class="text-xs font-mono opacity-60 mb-1"> RL </div>
  <div class="font-bold mb-2">会做题</div>
  <div class="text-xs opacity-70 mb-2">奖励好的回答、惩罚坏的回答</div>
  ...
  <div class="text-[11px] mt-2 opacity-60"> 数学、代码等可验证的任务</div>
</div>
...
各家厂商在这三个环节做了不同的取舍，这也就是为什么 ChatGPT / Claude / DeepSeek 能力不同，说话风格各有各的"口癖"
</div>

<SlideRefs :refs="[
  { label: 'Where the goblins came from', url: 'https://openai.com/index/where-the-goblins-came-from/' }
]" />
...
- 包括系统提示、历史消息、你的问题等 —— 全部拼在一起送给大模型进行推理
- 有长度上限（128K、200K 等），超出就需要 compact
- **不是"记忆"**：每次请求都从零开始
...
<div v-click class="mt-7">
  <div class="text-xs mb-1 opacity-60">一次对话中 Context 的组成（示例）</div>
  <div class="w-full h-8 rounded-lg flex overflow-hidden text-xs text-white font-medium">
    <div class="bg-purple-500 flex items-center justify-center" style="width: 15%">系统提示 Prompt</div>
    <div class="bg-teal-500 flex items-center justify-center" style="width: 30%">历史对话等</div>
...
<strong>Context Rot</strong>：输入 Token 越多，模型表现可能会下降 —— 注意力会被废话稀释。<br/>所以 Context 管理很重要。
</div>
...
<div v-click class="p-4 bg-gray-50 rounded-lg border border-gray-200 text-center">
  ...
  <div class="text-xs opacity-70">Cursor · VSCode Copilot</div>
</div>
...
<div v-click>
  <img src="/images/chat.png" class="rounded border border-gray-200 shadow-sm w-full" />
</div>

</div>

<div v-click class="mt-4 p-2 bg-gray-50 rounded-lg border border-gray-200 text-sm text-center">
模型的潜力被交互形式限制了 —— 它能写出正确的命令，但没法自己执行
</div>
...
<div class="mt-16 p-6 bg-teal-50 rounded-lg border border-teal-200 text-2xl font-medium text-center">
...
<div v-click>
<strong>原理其实很简单</strong>：模型可以生成"命令"，然后本地的程序负责执行，并把结果返回给它。
</div>

<div v-click>
模型的训练语料包含大量 CLI 命令（<code>grep</code>、<code>cat</code>、<code>find</code> 等），所以它天然会生成这些命令 —— Agent 负责执行。
</div>
...
%%{init: {'sequence': {'actorMargin': 180, 'width': 200, 'boxMargin': 4, 'messageMargin': 28, 'noteMargin': 6, 'mirrorActors': false}}}%%
sequenceDiagram
    autonumber
    participant U as 用户
    ...
    A->>L: [系统提示 + 工具列表 + 用户请求]
    Note over L: ⏳ 推理中…
    L-->>A: Tool Call: read_file("report.txt")
    Note over A: ⚙ 执行工具：读取文件内容
    A->>L: [历史 context + 文件内容]
    Note over L: ⏳ 推理中…
    L-->>A: Tool Call: write_file("summary.txt", "...")
...
<div>
  <img src="/images/copilot.png" class="rounded border border-gray-200 shadow-sm w-full" />
</div>
...
<div class="grid gap-6 mt-4 items-start" style="grid-template-columns: 1fr 1.336fr">

<div class="space-y-2 text-sm">

<div class="p-2 bg-amber-50 rounded-lg border border-amber-200 text-xs">
<strong>侧边栏模式的局限</strong><br/>
交互效率低<br/>
阻碍自动化<br/>
不支持长时任务<br/>
无法并行多 Agent
...
<div v-click class="mt-2 p-2 bg-gray-50 rounded border border-gray-200 text-xs italic">
...
<div>
  <img src="/images/claude.jpg" class="rounded border border-gray-200 shadow-sm w-full" />
</div>
...
<div class="grid gap-6 mt-4 items-start" style="grid-template-columns: 1fr 1.5fr">

<div class="space-y-2 text-sm">

<v-clicks>

- AI Agent 需要和外部世界/工具交互
- MCP 提供了标准化的方式来定义这些"工具"
...
<div v-click>
  <img src="/images/mcp.avif" class="rounded border border-gray-200 shadow-sm w-full" />
</div>

</div>

<div v-click class="mt-4 p-3 bg-amber-50 rounded-lg border border-amber-200 text-sm">
<strong>为什么UE需要 MCP？</strong> —— 理由之一是，Agent 不认识UE的二进制格式。<br/>UE 的 .uasset 对 AI 就是乱码，AI无法原生理解uasset的格式，只能通过引擎来操作：Agent → 引擎 MCP → .uasset
</div>
...
每次请求把 <strong>系统提示 + 历史对话 + 本轮 tool call 结果</strong> 全部送回<br/>
· Context 越长，耗时和费用都会非线性增长<br/>
· Context 越多，每条信息被正确利用的概率越低 —— AI 会 Lost in the middle
</div>

<div v-click class="p-3 bg-teal-50 rounded-lg border border-teal-200">
利用好模型的注意力<br/>
· <strong>精简注入</strong>（只提供真正需要的信息）<br/>
· <strong>控制边界</strong>（拆好任务，逐个交付）
</div>
</div>
...
<div class="grid grid-cols-2 gap-5 mt-4 items-start text-sm">
...
<strong>TAPD 单子</strong><br/>
<span class="text-xs opacity-70">需求背景、优先级、复现步骤</span>
</div>

<div class="p-3 bg-gray-100 rounded-lg border border-gray-200">
<strong>策划案</strong><br/>
<span class="text-xs opacity-70">设计意图、数值逻辑、改动原因</span>
</div>

<div class="p-3 bg-gray-100 rounded-lg border border-gray-200">
<strong>会议纪要</strong><br/>
<span class="text-xs opacity-70">对齐结论、口头约定 —— 说完就消失</span>
</div>

<div class="p-3 bg-gray-100 rounded-lg border border-gray-200">
<strong>代码 / 工具的文档</strong><br/>
<span class="text-xs opacity-70">接口说明、CLI 命令、工程约定</span>
</div>
...
<div v-click class="p-3 bg-gray-50 rounded-lg border border-gray-200">
解决方案：把知识推入 repo，用<strong>结构化文档</strong>取代口头沟通
</div>
...
<span class="text-xs">短小的入口文件（如 <code>AGENTS.md</code>）作为地图，指向更深的 <code>docs/</code> —— 先给概要，需要时再取细节</span>
</div>
...
什么样的任务适合交给 Agent？ Well-defined problems
...
问题有明确封闭的边界：上下文信息有清晰的边界，避免撑爆上下文
...
游戏是给人的，核心是互动 —— 是给人的 <u>多模态能力</u> 设计的
...
<div v-click class="mt-3 p-3 bg-teal-50 rounded-lg border border-teal-200 text-sm text-center max-w-lg mx-auto">
UE 是给 <strong>人</strong> 用的 —— AI 时代需要想想给 <strong>Agent</strong> 怎么设计接口
</div>
...
<div v-click class="p-3 bg-teal-50 rounded-lg border-l-4 border-teal-400">
<strong>uasset 生成</strong> —— binary格式，需要改代码 + MCP 封装，只能case by case 讨论（例如json化）
</div>
...
引擎基建需要向 AI 时代靠拢。AI 放大的是已有的基建质量 —— <span v-mark="{ at: 4, type: 'underline', color: '#10b981' }">基建越Agent友好，杠杆越大</span>
</div>
...
<SlideRefs :refs="[
  { label: 'WORLD MODELS, INTERACTIVE ENTERTAINMENT, AND THE PRIMACY OF STRUCTURE IN GAME DESIGN', url: 'https://x.com/eric_seufert/status/2017624994096750686' }
]" />
...
LLM 不一样：<span v-mark="{ at: 3, type: 'highlight', color: '#14b8a6' }">很弱和很强紧挨在一起</span>，能力分布是 <strong>锯齿状</strong>的 —— 跟人打交道形成的直觉，套到 LLM 上不一定适用。
</div>
...
<div class="font-bold text-base mb-1">1. 不 FOMO，自己体验，多 Build</div>
<div class="text-sm">以月为单位看下最强模型的能力，少看某些公众号的的标题党，多上手试。</div>
...
<div class="text-sm">让基础设施变得 <strong>agent friendly</strong>，才能让 Agent 发挥最大的作用。</div>
<div class="text-sm">迎接 <strong>工具民主化 </strong>的时代，Agent 作为队友的时代，AI 无处不在的时代。</div>
...
<div class="text-sm">Token 是代币，AI 是老虎机 —— Vibe Coding 容易变得和打游戏一样上瘾，AI 是概率系统，但人脑喜欢确定性。</div>
<div class="text-sm mt-1">同时管太多 AI，人脑要一直 Context Switch，很累。</div>
</div>
...
4. 整理谢天这份 slides 里所有的 ref，适合我阅读吗？我刚兴趣的是 xxx，能推荐点更多的资料吗？
