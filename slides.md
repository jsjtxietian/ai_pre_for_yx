...
<strong>为什么需要 MCP？</strong> —— 理由之一是，Agent 不认识二进制格式。<br/>UE 的 .uasset 对 AI 就是乱码，Agent 只能通过引擎来操作：Agent → 引擎 MCP → .uasset
</div>
...
Feedback Loop很难。一个玩法尚且如此，更别提加上美术、关卡设计、数值。
</div>
...
<div class="text-sm">以月为单位看下最强模型的能力，少看 <span class="font-mono text-xs bg-white/70 px-1 rounded">机器之心 / 量子位 / 新智元</span> 的标题党，多上手试。</div>
...
<div class="text-sm">Token 是代币，AI 是老虎机 —— Vibe Coding 容易变得和打游戏一样上瘾。</div>
<div class="text-sm mt-1">我们在和一个 <strong>概率系统</strong> 协作，但大脑其实习惯 <strong>确定性系统</strong> —— 这种错配会造成压力。</div>
</div>
