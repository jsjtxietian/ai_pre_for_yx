...
多请求并行时计算顺序不固定，微小差异同样层层放大（动态 Batching、量化、专家丢弃）。
...
- 有长度上限（128K、200K Token 等），超出就需要 compact
- **不是"记忆"**：每次请求都从零开始
...
<strong>为什么需要 MCP？</strong> —— Agent 不认识二进制格式。<br/>UE 的 .uasset 对 AI 就是乱码，Agent 只能通过引擎来操作：Agent → 引擎 MCP → .uasset
</div>
...
<div v-click class="p-3 bg-teal-50 rounded-lg border border-teal-200">
利用好模型的注意力<br/>
· <strong>精简注入</strong>（只放本轮需要的）<br/>
· <strong>控制边界</strong>（子 Agent / 分轮）
</div>
</div>
